...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!claimed)
        return res.status(400).send({
          message: "You have already claimed today's daily plus gold",
        });
...
router.get("/:steamID/stats", auth.checkDoNotTrack, async (req, res) => {
  try {
    const steamID = req.params.steamID;
...
router.get("/:steamID/games", auth.checkDoNotTrack, async (req, res) => {
  try {
    const steamID = req.params.steamID;
...
router.get("/:steamID/login_quests", auth.checkDoNotTrack, async (req, res) => {
  try {
    const steamID = req.params.steamID;
    const shouldTrack = await players.getShouldTrackData(steamID);
    if (!shouldTrack) {
