<template>
  <div>
    ...
    <h5 v-if="!loading" style="text-align: center">
      Viewing stats for {{ totalGames.toLocaleString() }} games
    </h5>
...
export default {
  ...
  created() {
    this.dateOptions = [
      {
        value: { startDate: "2022-09-01", endDate: this.daysAgo(0) },
        text: "All Time",
      },
      { value: null, text: "------------", disabled: true },
      {
        value: { startDate: this.daysAgo(7), endDate: this.daysAgo(0) },
        text: "Week",
      },
      {
        value: { startDate: this.daysAgo(30), endDate: this.daysAgo(0) },
        text: "Month",
      },
      {
        value: { startDate: this.daysAgo(365), endDate: this.daysAgo(0) },
        text: "Year",
      },
      { value: null, text: "------------", disabled: true },
      // TODO: Pull these from common/patches.js
      {
        value: { startDate: "2022-07-08", endDate: "2022-11-01" },
        text: "Season 1",
      },
      {
        value: { startDate: "2022-10-05", endDate: "2022-11-01" },
        text: "Season 1.5",
      },
      {
        value: { startDate: "2022-11-01", endDate: "2022-12-01" },
        text: "Season 2",
      },
      {
        value: { startDate: "2022-11-14", endDate: "2022-12-01" },
        text: "Season 2.5",
      },
      {
        value: { startDate: "2022-12-01", endDate: "2023-01-01" },
        text: "Season 3",
      },
      {
        value: { startDate: "2022-12-15", endDate: "2023-01-01" },
        text: "Season 3.5",
      },
      {
        value: { startDate: "2023-01-01", endDate: "2023-03-01" },
        text: "Season 4",
      },
      {
        value: { startDate: "2023-02-01", endDate: "2023-03-01" },
        text: "Season 4.5",
      },
      {
        value: { startDate: "2023-03-01", endDate: "2023-05-01" },
        text: "Season 5",
      },
      { value: null, text: "------------", disabled: true },
    ...
    fetch(`/data/gods.json`)
      ...
      .then((d) => {
        this.godData = d;
        this.godUnlockOptions = [
          "All",
          ...new Set(
            d.map(
              (gd) =>
                gd.unlock?.charAt(0)?.toUpperCase() +
                gd.unlock?.slice(1).toLocaleLowerCase()
            )
          ),
        ];
      });
  ...
  methods: {
    loadGodStats() {
      ...
      fetch(
        ...
        .then((gods) => {
          ...
          this.totalGames = Math.round(
            this.filteredGods.reduce(
              (sumGames, cur) => sumGames + Number(cur.picks),
              0
            ) / 8
          );
          this.loading = false;
    ...
    filterGods() {
      ...
      if (this.selectedUnlock == "All") {
        ...
        this.filteredGods = this.gods.filter(
          (g) =>
            this.godData
              .find((gd) => gd.id == g.god_name)
              .unlock.toLocaleLowerCase() ==
            this.selectedUnlock.toLocaleLowerCase()
        );
      }
      this.totalGames = Math.round(
        this.filteredGods.reduce(
    ...
    daysAgo(days) {
      ...
      return `${d.toLocaleString("en-US", {
        year: "numeric",
      })}-${d.toLocaleString("en-US", { month: "numeric" })}-${d.toLocaleString(
        "en-US",
        { day: "numeric" }
      )}`;
    },
