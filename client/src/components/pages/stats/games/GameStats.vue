<template>
  <div>
    ...
    <GameLineGraph
      :chart-data="gamesPlayedData"
      :loading="loadingCharts"
      :title="gamesPlayedTitle"
    />
    <GameLineGraph
      :chart-data="roundsData"
      :loading="loadingCharts"
      :title="roundsTitle"
    />
    <GameLineGraph
...
export default {
  ...
  methods: {
    loadGameStats() {
      ...
      fetch(`/api/stats/games?startDate=${startDate}&endDate=${endDate}`)
        .then((res) => res.json())
        .then((gameRows) => {
          ...
          this.averageDuration = `${durationMinutes}m ${durationSeconds.toFixed(
            0
          )}sec`;
